...
app.post('/api/verify/toggle', (req, res) => {
  ...
  if (idx > -1) { DB.verified.splice(idx, 1); res.json({ verified: false }); }
  else { DB.verified.push(target); res.json({ verified: true }); }
});

app.get('/api/verified', (req, res) => res.json(DB.verified));
...
app.post('/api/bank/click', (req, res) => { const b = DB.bank[req.body.username]; b.balance += b.clickLevel; res.json({ balance: b.balance }); });
app.post('/api/bank/upgrade', (req, res) => { const b = DB.bank[req.body.username]; const cost = b.clickLevel * 2 * 50; if (b.balance < cost) return res.json({ error: 'недостаточно средств' }); b.balance -= cost; b.clickLevel++; res.json({ balance: b.balance, clickLevel: b.clickLevel }); });
app.post('/api/bank/passive', (req, res) => { const b = DB.bank[req.body.username]; const cost = (b.passiveLevel + 1) * 100; if (b.balance < cost) return res.json({ error: 'недостаточно средств' }); b.balance -= cost; b.passiveLevel++; res.json({ balance: b.balance, passiveLevel: b.passiveLevel }); });
app.post('/api/bank/deposit', (req, res) => { const b = DB.bank[req.body.username]; const amount = parseInt(req.body.amount); if (!amount || amount < 100) return res.json({ error: 'мин. 100 🪙' }); if (b.balance < amount) return res.json({ error: 'недостаточно средств' }); b.balance -= amount; b.depositAmount = amount; b.depositTimer = Date.now(); res.json({ success: true }); });
app.post('/api/bank/withdraw', (req, res) => { const b = DB.bank[req.body.username]; if (!b.depositAmount) return res.json({ error: 'нет вклада' }); const amount = b.depositAmount; b.balance += amount; b.depositAmount = 0; b.depositTimer = 0; res.json({ success: true }); });
app.post('/api/bank/savings/deposit', (req, res) => { const b = DB.bank[req.body.username]; const amount = parseInt(req.body.amount); if (!amount || amount < 500) return res.json({ error: 'мин. 500 🪙' }); if (b.balance < amount) return res.json({ error: 'недостаточно средств' }); b.balance -= amount; b.savingsBalance = (b.savingsBalance || 0) + amount; res.json({ success: true }); });
app.post('/api/bank/savings/withdraw', (req, res) => { const b = DB.bank[req.body.username]; if (!b.savingsBalance) return res.json({ error: 'нет сбережений' }); const amount = b.savingsBalance; b.balance += amount; b.savingsBalance = 0; res.json({ success: true }); });
app.post('/api/bank/transfer', (req, res) => { const { from, to, amount } = req.body; const s = DB.bank[from], r = DB.bank[to]; if (!s || !r) return res.json({ error: 'пользователь не найден' }); if (s.balance < amount) return res.json({ error: 'недостаточно средств' }); s.balance -= amount; r.balance += amount; if (!DB.chats[to]) DB.chats[to] = {}; if (!DB.chats[from]) DB.chats[from] = {}; const msg = { from, text: `💰 перевёл ${amount} 🪙`, time: new Date().toLocaleTimeString() }; if (!DB.chats[to][from]) DB.chats[to][from] = []; if (!DB.chats[from][to]) DB.chats[from][to] = []; DB.chats[to][from].push(msg); DB.chats[from][to].push(msg); res.json({ success: true }); });
...
// Gray AI
app.post('/api/grayai', (req, res) => {
  const { prompt } = req.body;
  const responses = [
    `Интересный вопрос! Вот что я думаю: "${prompt}" — это любопытно. Возможно, стоит подумать об этом с другой стороны.`,
    `Привет! Я Gray AI. По поводу "${prompt}": я бы посоветовал исследовать эту тему глубже.`,
    `Отличная тема! "${prompt}" заслуживает внимания. Мой ответ: всё зависит от контекста.`,
    `Я analysed ваш запрос "${prompt}". Результат: это имеет потенциал. Попробуйте!`,
    `Хм, "${prompt}" — звучит как что-то из 2026 года! Я думаю, будущее за этим.`
  ];
...
module.exports = app;
if (require.main === module) app.listen(process.env.PORT || 3000, () => console.log('Zocial OK (c) 2026 zima.dev')));
