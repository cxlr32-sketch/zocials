...
app.post('/api/login', async (req, res) => {
  ...
  res.json({ ok: true, user: { name: u.name, username, avatar: u.avatar, bio: u.bio } });
});
...
app.post('/api/session', async (req, res) => {
  ...
  res.json({ ok: true, user: { name: u.name, username: req.body.username, avatar: u.avatar, bio: u.bio } });
});
...
app.post('/api/posts', async (req, res) => {
  ...
  const post = {
    id: 'p' + Date.now(), username, text: text || '', media: media || null, media_type: mediaType || null,
    time: 'только что', timestamp: Date.now(), likes: 0, liked_by: [], comments: []
  };
...
app.post('/api/grayai', async (req, res) => {
  const r = ['Интересный вопрос!', 'Я думаю, это зависит от ситуации.', 'Хорошая тема для размышления.'];
